...
export const useVaultApprove = (setLastUpdated: () => void) => {
  ...
  const handleApprove = () => {
    cakeContract.methods
      ...
      .on('receipt', () => {
        toastSuccess(t('Contract Enabled'), t('You can now stake in the %symbol% vault!', { symbol: 'WEX' }))
        setLastUpdated()
        setRequestedApproval(false)
      })
      .on('error', (error) => {
...
export const useRedeemApprove = () => {
  const { account } = useWeb3React()
  const redeemContract = usePresale()
  const usdtContract = useUSDT()
  const handleApprove = useCallback(async () => {
    try {
      const tx = await approve(usdtContract,redeemContract, account)
      return tx
    } catch (e) {
      return false
    }
  }, [account, usdtContract, redeemContract])
