...
const getProfile = async (address: string): Promise<GetProfileResponse> => {
  try {
    ...
    const { userId, tokenId, nftAddress, isActive } = transformProfileResponse(profileResponse)
    const team = await getTeams(address)
    ...
    let nft: Nft
    if (isActive) {
      nft = await getNft(nftAddress, tokenId)
      ...
      Cookies.set(
        ...
        {
          username,
          avatar: `https://wedex.app/images/nfts/${nft?.images.sm}`,
        },
        { domain: 'wedex.app', secure: true, expires: 30 },
      )
    }
