...
import { fetchTeams, fetchReferrals } from './teams'
...
export const usePollCoreFarmData = () => {
  ...
  }, [dispatch, fastRefresh,account,pids])
}
...
export const useFarms = (): FarmsState => {
  return useSelector((state: State) => state.farms)
}
...
export const useReferrals = (accounts) => {
  const {isInitialized, isLoading, data}: TeamsState = useSelector((state: State) => state.teams);
  const dispatch = useAppDispatch()
  useEffect(() => {
    if (accounts) {
      dispatch(fetchReferrals(accounts))
    }
  }, [accounts, dispatch])
...
export const usePriceCakeBusd = (): BigNumber => {
  const wexBusdFarm = useFarmFromPid(1)
  return new BigNumber(wexBusdFarm.token.busdPrice)
}
