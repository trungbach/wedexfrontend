...
const ResendActivationModal: React.FC<ForgotPasswordModalProps> = ({ onDismiss }) => {
  const { t } = useTranslation()
  const onResendActivation = useResendActivation
  const recaptchaRef = React.createRef()
  const [email, setEmail] = useState('')
  const [toasts, setToasts] = useState([])
  const [recaptcha, setRecaptcha] = useState('')

  const handleResendActivation = async (_email: string, _recaptcha: string) => {
    const response = await onResendActivation(_email, _recaptcha)
  ...
  return (
    <Modal title={t('Forgot Password')} onDismiss={onDismiss}>
      ...
      <Button style={{ marginBottom: '10px' }} onClick={async () => {
        await handleResendActivation(email, recaptcha)
