...
export const getNft = async (nftAddress: string, tokenId: number): Promise<Nft | null> => {
  const contract = getErc721Contract(nftAddress)
  const variationId = await contract.methods.getBunnyId(tokenId).call()
  ...
  return Nfts.find((nft) => {
    return nft.variationId === parseInt(variationId)
  })
}
